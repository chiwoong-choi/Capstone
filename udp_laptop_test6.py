...
def get_monotonic_timestamp(counter):
    return counter * 10000  # 1만 단위로 증가 (마이크로초 유사)

def send_frame(socket, frame, counter):
    _, buffer = cv2.imencode('.jpg', frame)
    data = buffer.tobytes()
    size = len(data)
    timestamp = get_monotonic_timestamp(counter)
