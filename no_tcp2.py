...
def process_frame(camera_type, frame, timestamp, pose, silhouette_cache, silhouette_angle, body_parts, colors, previous_knee_angle=None, previous_silhouette=None, frame_idx=None, angle_threshold=5):
    ...
    global knee_angle_history
    knee_angle_history.append(knee_angle)
    if len(knee_angle_history) > 5:
        knee_angle_history.pop(0)
    smooth_knee_angle = np.mean(knee_angle_history)
    
    # 실루엣 선택
    silhouette = None
    silhouette_keys = list(data_dict.keys())  # ["0000.png", ...]
    silhouette_angle = np.array([v[2] for v in data_dict.values()])

    closest_idx = np.argmin(np.abs(silhouette_angle - knee_angle))
    silhouette_key = silhouette_keys[closest_idx]  # 예: "0023.png"
    base_idx = os.path.splitext(silhouette_key)[0]  # "0023"

    silhouette = silhouette_cache.get(base_idx, {})
    silhouette['idx'] = base_idx
