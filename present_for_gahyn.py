...
def create_silhouettes(front_input_dir, right_input_dir, front_silhouette_dir, right_silhouette_dir):
    ...
    for img_name in front_image_files:
        ...
        silhouette_frame[binary_mask > 0] = (255, 0, 0)  # 실루엣을 파란색으로 표시
    ...
    for img_name in right_image_files:
        ...
        # 실루엣 생성
        silhouette_frame = np.zeros_like(frame)    
        silhouette_frame[binary_mask > 0] = (255, 0, 0)  # 실루엣을 빨간색으로 표시
...
def realtime_synchronize(data_dict, front_camera_thread, right_camera_thread):
    ...
    num_silhouettes = len(silhouette_angle)
    print(f"✅ [INFO] 데이터 로드 완료, 실루엣 개수: {num_silhouettes}")
    ...
    while True:
        ...
        if results_front and results_front.pose_landmarks:
            ...
            front_silhouette_resized = cv2.resize(front_silhouette, (frame_front.shape[1], frame_front.shape[0]))
            right_silhouette_resized = cv2.resize(right_silhouette, (frame_right.shape[1], frame_right.shape[0]))

            front_overlay = cv2.addWeighted(frame_front, 0.7, front_silhouette_resized, 0.3, 0)
            right_overlay = cv2.addWeighted(frame_right, 0.7, right_silhouette_resized, 0.3, 0)

            cv2.putText(front_overlay, f"Squat Count: {squat_count}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.putText(right_overlay, f"Squat Count: {squat_count}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):  # 10ms 딜레이
