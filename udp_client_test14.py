        ...
        while self.running:
            try:
                ...
                while len(data) < size:
                    ...
                    if not packet:
                        print("⚠️ [WARNING] 프레임 데이터 수신 중 연결 끊김")
                        break
                    data += packet

                if len(data) != size:
                    print(f"⚠️ [WARNING] 데이터 크기 불일치 (예상: {size}, 수신: {len(data)})")
                    continue

                frame = cv2.imdecode(np.frombuffer(data, np.uint8), cv2.IMREAD_COLOR)
                if frame is None:
                    print("⚠️ [WARNING] 디코딩 실패 - 빈 프레임 무시")
                    continue
                ...
                print(f"⚠️ [WARNING] 프레임 수신 중 오류: {e}")
                break
...
def process_frame(camera_type, frame, timestamp, pose, silhouette_cache, silhouette_angle, body_parts, colors, previous_knee_angle=None, previous_silhouette=None, angle_threshold=5):
    """
    단일 프레임을 처리하는 함수.
...
def realtime_synchronize(camera_type, data_dict, front_camera_thread, right_camera_thread, reps=1, sets=3, current_set=1):
    ...
    카운팅/정확도/경고 등은 process_frame에서 전역 변수로 관리.
    """
    ...
    while True:
        try:
            ...
            if front_frame is None or front_timestamp is None:
                print("⚠️ [WARNING] 전면 프레임 없음, 건너뜀")
                time.sleep(0.01)
                continue
            if right_frame is None or right_timestamp is None:
            ...
            future_front = executor.submit(
                process_frame, "front", front_frame, front_timestamp, pose, front_silhouette_cache,
                silhouette_angle, body_parts, colors, previous_knee_angle_front, previous_silhouette_front
            )
            future_right = executor.submit(
                process_frame, "right", right_frame, right_timestamp, pose, right_silhouette_cache,
