...
#5/19 수정: 실루엣 사진 자체 저장 x, npy파일 저장 후 캐싱하여 사용
#5/21 수정: squat_end에서 결과 분석용 코드 작성
import cv2
import os
import time
import csv
...
def cache_silhouette_npy(silhouette_dir, body_parts):
    cache = {}
    frame_indices = set()
...
# 서버 시작 시 캐싱
front_silhouette_cache = cache_silhouette_npy(front_silhouette_dir, ["head","left_arm","right_arm","left_leg","right_leg"])
right_silhouette_cache = cache_silhouette_npy(right_silhouette_dir, ["head","left_arm","right_arm","left_leg","right_leg"])

def process_frame(camera_type, frame, timestamp, pose, silhouette_cache, silhouette_angle, body_parts, colors, previous_knee_angle=None, previous_silhouette=None, frame_idx=None, angle_threshold=5):
    """
    단일 프레임을 처리하는 함수.
    실루엣 오버레이, 스쿼트 카운팅, 경고, 정확도 누적까지 모두 담당.
    """
    if frame is None:
        print("⚠️ [WARNING] process_frame에 None 프레임 전달됨, 무시")
        return None, previous_knee_angle, previous_silhouette
    global mismatch_frame_count, total_frame_count
    global squat_count, is_squatting, warned_low_depth, not_deep_squat, squat_count_status
    ...
    if frame_idx is not None:
        silhouette = silhouette_cache.get(f"{frame_idx:04d}", {})
        ...
        if previous_knee_angle is not None and abs(knee_angle - previous_knee_angle) <= angle_threshold:
            ...
            silhouette = silhouette_cache.get(f"{closest_frame_index:04d}", {})
            previous_silhouette = silhouette
    previous_knee_angle = knee_angle

    # 실루엣 오버레이 및 mismatch 분석
    overlay = frame.copy()
    mismatch_parts = []
    for part, indices in body_parts.items():
        try:
    ...
    if knee_angle > 140:
        ...
        if not warned_low_depth:
            print("⚠️ [WARNING] 스쿼트 깊이가 부족합니다! 더 내려가세요.")
...
def realtime_synchronize(camera_type, data_dict, front_camera_thread, right_camera_thread, reps=1, sets=3, current_set=1):
    
    print("🔵 [INFO] 실루엣 동기화 시작")
    global squat_count, not_deep_squat
    ...
    while True:
        try:
            ...
            ret_front, front_frame = front_camera.read()
            ret_right, right_frame = right_camera.read()
            front_timestamp = None  # 필요 없다면 None 또는 time.time() 등으로 대체
            right_timestamp = None
            now = time.time()
            if now - last_frame_time > 2.0:
                print("🔄 [INFO] 2초 이상 프레임 없음, MediaPipe pose 객체 재생성")
                pose_front = mp_pose.Pose(
                    static_image_mode=False,
                    model_complexity=0,
                    smooth_landmarks=True,
                ...
                last_frame_time = now  # 재생성 후 시간 초기화
            # 프레임 가져오기
            

            if not ret_front or front_frame is None:
                print("⚠️ [WARNING] 전면 프레임 없음, 건너뜀")
                time.sleep(0.01)
                continue
            if not ret_right or right_frame is None:
                print("⚠️ [WARNING] 측면 프레임 없음, 건너뜀")
                time.sleep(0.01)
                continue
            
            last_frame_time = now  # 프레임을 정상적으로 받았으므로 시간 갱신
            
            # 병렬 처리
            future_front = executor.submit(
                process_frame, "front", front_frame, front_timestamp, pose_front, front_silhouette_cache,
                silhouette_angle, body_parts, colors, previous_knee_angle_front, previous_silhouette_front, frame_idx_front
            )
            future_right = executor.submit(
                process_frame, "right", right_frame, right_timestamp, pose_right, right_silhouette_cache,
                silhouette_angle, body_parts, colors, previous_knee_angle_right, previous_silhouette_right, frame_idx_right
            )
            ...
            # 세트 종료 조건
            if squat_count >= reps:
                total_squat_count = not_deep_squat + squat_count
                squat_accuracy = squat_count / total_squat_count * 100 if total_squat_count > 0 else 0
                session['squat_accuracy'] = squat_accuracy
                session['current_set'] = current_set + 1
                break
