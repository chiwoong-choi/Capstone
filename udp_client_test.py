...
class UDPFrameReceiver:
    def __init__(self, udp_ip, udp_port):
        """
        UDP 소켓을 통해 프레임을 수신하는 클래스.
        
        Args:
            udp_ip (str): 수신할 IP 주소.
            udp_port (int): 수신할 포트 번호.
        """
        self.udp_ip = "0.0.0.0"
    ...
    def update(self):
        """
        UDP 소켓을 통해 지속적으로 프레임을 수신하는 메서드.
        """
        while self.running:
            try:
                data, _ = self.socket.recvfrom(65536)
                timestamp, frame_data = struct.unpack("d", data[:8])[0], data[8:]
            ...
            time.sleep(0.01)  # CPU 점유율을 낮추기 위해 잠시 대기
...
# ------------------- 동기화 함수 -------------------
def get_synchronized_frames(front_camera, right_camera, max_time_diff=0.05):
    """
    두 카메라의 프레임을 동기화하여 반환합니다.
    
    Args:
        front_camera (UDPFrameReceiver): 전면 카메라 수신기.
        right_camera (UDPFrameReceiver): 측면 카메라 수신기.
        max_time_diff (float): 최대 허용 타임스탬프 차이 (초).
    ...
    # 타임스탬프 동기화 확인
    if abs(front_timestamp - right_timestamp) <= max_time_diff:
...
# return된 변수 -> squat_check 결과창에 표시해야댐
...
# 동기화된 프레임 저장
front_camera = UDPFrameReceiver("0.0.0.0", 5005)  # 전면 카메라 포트
right_camera = UDPFrameReceiver("0.0.0.0", 5006)  # 측면 카메라 포트
